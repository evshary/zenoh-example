...
int main(void) {
    ...
    const char *keyexpr = "demo/example/zenoh-pico-simple-advanced-pubsub";
    const char *value = "Hello from advanced zenoh-pico!";
    const int publish_count = 10;

    // ----- key expression -----
    // Create key expression for advanced publisher and subscriber
    z_view_keyexpr_t ke;
    ...
    if (z_open(&sub_session, z_move(sub_config), NULL) < 0) {
        printf("Unable to open advanced subscriber session.\n");
        return -1;
    }
    ...
    ze_advanced_subscriber_options_t sub_options;
    ze_advanced_subscriber_options_default(&sub_options);

    ze_owned_advanced_subscriber_t subscriber;
    if (ze_declare_advanced_subscriber(z_loan(sub_session), &subscriber, z_loan(ke), z_move(callback),
                                       &sub_options) < 0) {
    ...
    if (z_open(&pub_session, z_move(pub_config), NULL) < 0) {
        printf("Unable to open advanced publisher session.\n");
        z_drop(z_move(subscriber));
        z_drop(z_move(sub_session));
        return -1;
    }

    ze_advanced_publisher_options_t pub_options;
    ze_advanced_publisher_options_default(&pub_options);
    pub_options.cache.is_enabled = true;
    pub_options.cache.max_samples = 1;

    ze_owned_advanced_publisher_t publisher;
    if (ze_declare_advanced_publisher(z_loan(pub_session), &publisher, z_loan(ke), &pub_options) < 0) {
    ...
    // Wait a little for the routed samples to arrive, but do not hang forever
    // if the router is unavailable or the subscriber did not match in time.
    int wait_loops = 0;
